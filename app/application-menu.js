...
module.exports = () => {
    const hasWindows = !!BrowserWindow.getAllWindows().length; // 判断是否还存在着窗口
    const focusedWindow = BrowserWindow.getFocusedWindow(); // 判断聚焦的是哪个窗口
    const hasFilePath = !!(focusedWindow && focusedWindow.getRepresentedFilename()); // 从聚焦的窗口中获取文件路径
    const template = [
        {
            label: '文件',
            submenu: [
                ...
                // 以下菜单内容在窗口丢失焦点的时候，都会出问题
                {
                    ...
                    click(item, focusWindow) {
                        if (focusWindow) {
                            ...
                            ipcMain.emit('select-file-open-new-window');
                        }
                    }
                },
                {
                    ...
                    click(item, focusWindow) {
                        focusWindow.webContents.send('invoke-save-markdown',);
                    }
                },
                {
                    label: '导出HTML',
                    ...
                    enabled: !!focusedWindow,
                    click(item, focusWindow) {
                        focusWindow.webContents.send('invoke-save-html');
                    }
                },
                {type: 'separator'},
                {
